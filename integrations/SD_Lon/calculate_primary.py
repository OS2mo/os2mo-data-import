...
from integrations.SD_Lon import sd_common, sd_payloads
...
class MOPrimaryEngagementUpdater(object):
    ...
    def _calculate_rate_and_ids(self, mo_engagement, no_past):
        ...
        for eng in mo_engagement:
            ...
            logger.debug(stat.format(max_rate, min_id, employment_id, eng["fraction"]))
    ...
    def _find_cut_dates(self, no_past=False):
        ...
        dates = set()
        for eng in mo_engagement:
            dates.add(datetime.datetime.strptime(eng["validity"]["from"], "%Y-%m-%d"))
            if eng["validity"]["to"]:
                to = datetime.datetime.strptime(eng["validity"]["to"], "%Y-%m-%d")
