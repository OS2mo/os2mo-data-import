...
def relations_ret(virkning, pnummer=None, phone=None, adresse=None):
    ...
    relations_liste = {"sd:LokalUdvidelse": {"silkdata:Lokation": locations}}
...
def attributes_flyt(virkning, name):
    return {"Egenskab": {"sd:EnhedNavn": name, "sd:Virkning": virkning}}


def create_registrering(virkning, registry_type):
    if registry_type not in ("Rettet", "Opstaaet"):
        raise RuntimeError(
            "Integration, SD-løn: ukendt registreringstype: %s" % (registry_type,)
        )
    registrering = {
