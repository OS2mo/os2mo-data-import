...
class OpusDiffImport(object):
    def __init__(self, latest_date, ad_reader, employee_mapping={}):
        ...
        self.manager_types, self.manager_type_facet = self._find_classes("manager_type")
    ...
    def _update_manager_types(self, manager_type):
        ...
        if manager_type_uuid is None:
            ...
            response = self._add_klasse_to_lora(manager_type, self.manager_type_facet)
            uuid = response["uuid"]
    ...
    def _update_manager_level(self, manager_level):
        ...
        if manager_level_uuid is None:
            ...
            response = self._add_klasse_to_lora(manager_level, self.manager_level_facet)
            uuid = response["uuid"]
    ...
    def _find_engagement(self, bvn, present=False):
        info = {}
        resource = (
            "/organisation/organisationfunktion?bvn={}&funktionsnavn=Engagement".format(
                bvn
            )
        )
        if present:
        ...
        if uuids:
            if len(uuids) == 1:
                ...
                for uuid in uuids:
                    ...
                    org_funk_type = org_funk["attributter"][
    ...
    def _update_employee_address(self, mo_uuid, employee):
        ...
        for addr_type, setting in EMPLOYEE_ADDRESS_CHECKS.items():
            ...
            address_args = {
                ...
                "validity": {"from": self.latest_date.strftime("%Y-%m-%d"), "to": None},
                "user_uuid": mo_uuid,
            }
    ...
    def update_roller(self, employee):
        ...
        for opus_role in opus_roles:
            ...
            for mo_role in self.role_cache:
                ...
                if (
                    mo_role["person"] == mo_user["uuid"]
                    and combined_role == mo_role["role_type_text"]
                ):
                    found = True
    ...
    def update_employee(self, employee):
        ...
        if mo_user is None:
            ...
            if not (
                (employee["firstName"] == mo_user["givenname"])
                and (employee["lastName"] == mo_user["surname"])
            ):
                payload = payloads.create_user(
                    employee, self.org_uuid, employee_mo_uuid
                )
                return_uuid = self.helper._mo_post("e/create", payload).json()
