...
activity_log_handler = RotatingFileHandler(filename=LOG_FILE, maxBytes=1000000)
        ...
        payload = [
            {
                ...
                "validity": {"from": from_string, "to": None},
            }
        ]
...
def create_udvalg(nodes, file_name):
    ...
    for row in rows:
        ...
        org_id = int(row["Id"])
        uuid = _search_mo_name(row["Fornavn"] + " " + row["Efternavn"], row["BrugerID"])
        try:
            from_string = datetime.datetime.strftime(
                datetime.datetime.strptime(row["StartDato"], "%d-%b-%y"), "%Y-%m-%d"
            )
        except ValueError:
        ...
        if uuid:
            ...
            _create_mo_association(
                uuid, nodes[org_id].uuid, association_type, from_string
            )
            if role_type:
            ...
            logger.warning(
                "Error: {} {}, bvn: {}".format(
                    row["Fornavn"], row["Efternavn"], row["BrugerID"]
                )
            )
    return nodes
...
def create_tree(file_name):
    ...
    while rows:
        ...
        for row in rows:
            ...
            if parent is None:
                uuid = _create_mo_ou(
                    row["OrgEnhed"], parent="root", org_type=org_type, bvn=id_nr
                )
                new[id_nr] = Node(row["OrgEnhed"], uuid=uuid, org_type=org_type)
                ...
                new[id_nr] = Node(
                    row["OrgEnhed"], uuid=uuid, org_type=org_type, parent=nodes[parent]
                )
            else:
