...
def _read_primary_ad_settings(top_settings, index=0):
    ...
    primary_settings["servers"] = top_settings["integrations.ad"][index].get(
        "servers", []
    )
    primary_settings["caseless_samname"] = top_settings.get("integrations.ad")[
        index
    ].get("caseless_samname", True)
    primary_settings["sam_filter"] = top_settings.get("integrations.ad")[index].get(
        "sam_filter", ""
    )
    primary_settings["cpr_separator"] = top_settings.get("integrations.ad")[index].get(
        "cpr_separator", ""
    )
    primary_settings["ad_mo_sync_mapping"] = top_settings.get("integrations.ad")[
        index
    ].get("ad_mo_sync_mapping", {})
    primary_settings["method"] = top_settings["integrations.ad"][index].get(
        "method", "kerberos"
    )
...
def _read_primary_write_information(top_settings):
    ...
    # Shared with read
    primary_write_settings["cpr_field"] = top_settings.get("integrations.ad")[0][
        "cpr_field"
    ]
