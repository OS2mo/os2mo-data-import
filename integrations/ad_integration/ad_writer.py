...
from ad_template_engine import prepare_field_templates, template_powershell
from jinja2 import Template
from os2mo_helpers.mora_helpers import MoraHelper
from user_names import CreateUserNames
from utils import dict_exclude, dict_map, dict_subset, lower_list

from integrations.ad_integration.ad_exceptions import (
    CprNotFoundInADException, CprNotNotUnique,
    ManagerNotUniqueFromCprException, NoActiveEngagementsException,
    NoPrimaryEngagementException, ReplicationFailedException,
    SamAccountNameNotUnique, UserNotFoundException)
...
class ADWriter(AD):
    ...
    def _find_end_date(self, uuid):
        ...
        if self.lc_historic is not None:
            ...
            future_engagements = self.helper.read_user_engagement(
    ...
    def read_ad_information_from_mo(self, uuid, read_manager=True, ad_dump=None):
        ...
        manager_info = {"name": None, "sam": None, "mail": None, "cpr": None}
        if read_manager:
        ...
        if read_manager:
            ...
            try:
                manager_ad_info = self._find_ad_user(
                    cpr=manager_info["cpr"], ad_dump=ad_dump
                )
        ...
        mo_values = {
            ...
            "full_name": "{} {}".format(
                mo_user["givenname"], mo_user["surname"]
            ).strip(),
            "nickname": (mo_user["nickname_givenname"], mo_user["nickname_surname"]),
            "full_nickname": "{} {}".format(
                mo_user["nickname_givenname"], mo_user["nickname_surname"]
            ).strip(),
    ...
    def _sync_compare(self, mo_values, ad_dump):
        ...
        mo_values["name_sam"] = "{} - {}".format(
            mo_values["full_name"], ad["SamAccountName"]
        )
    ...
    def _cli(self):
        ...
        group.add_argument(
            "--add-manager-to-user", nargs=2, metavar=("Manager_SAM", "User_SAM")
        )
