...
class AposImport(object):
    ...
    def create_facetter_and_klasser(self):
        ...
        for k in klassifikationer:
            if k["@kaldenavn"] == "Stillingsbetegnelser":
                self.create_typer(
                    k["@uuid"], {"engagement_job_function": ["Alfabetisk"]}
                )
            if k["@kaldenavn"] == "Rolle":
                self.create_typer(k["@uuid"], {"role_type": ["Rolle"]})

            if k["@kaldenavn"] == "Enhedstyper":
                self.create_typer(k["@uuid"], {"org_unit_type": ["Alfabetisk"]})

            if k["@kaldenavn"] == "Tilknytningstyper":
                self.create_typer(k["@uuid"], {"engagement_type": ["Alfabetisk"]})
            ...
            if k["@kaldenavn"] == "Leder":
                self.create_typer(
                    k["@uuid"],
                    {"responsibility": ["Ansvar"], "manager_type": ["Typer"]},
                )

            if k["@kaldenavn"] == "SD løn enhedstyper":
                self.create_typer(
                    k["@uuid"], {"org_unit_type": ["sd_loen_enhedstyper"]}
                )

            if k["@kaldenavn"] == "SD løn enhedstyper":
                self.create_typer(k["@uuid"], {"time_planning": ["Tidsregistrering"]})
    ...
    def _create_ou_from_apos(self, apos_unit, parent=None, enhedstype=None):
        ...
        if details["opgaver"]:
            ...
            for opgave in opgaver:
                if opgave["@uuid"] in (
                    "41504f53-0203-0028-4158-41504f494e54",
        ...
        # Afdelingsniveau
        if enhedstype in (
            "81fc2195-d782-4ae0-8ab7-56a1f7ad92e2",
        ...
        for key, value in apos_int.items():
            ...
            self.importer.add_address_type(
                organisation_unit=unit_id, type_ref=key, value=value, date_from=fra
            )
        ...
        if int(associations["total"]) > 0:
            ...
            for person in persons:
                for p in person.values():
                    ...
                    details = self.importer.get_details(
                        owner_type="employee", owner_ref=p, type_id="engagement"
                    )
    ...
    def update_tasks(self, employee, unit):
        ...
        if not (
            self.importer.check_if_exists("klasse", stilling)
            and self.importer.check_if_exists("organisation_unit", unit)
            and self.importer.check_if_exists("employee", employee["person"]["@uuid"])
        ):
            ...
            for unit in org_units:
                ...
                if over_id in nodes.keys():
                    ...
                    new[unit_id] = self._create_ou_from_apos(
                        unit, over_id, enhedstype=enhedstype
                    )
