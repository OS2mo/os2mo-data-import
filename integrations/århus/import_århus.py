...
UNIT_EAN = (
    ...
    "EAN",
    "EANUnit",
    "500b06b0-39bd-4a50-962c-bc6f4ac63411",
)
UNIT_CVR = (
    "CVR nummer",
    "org_unit_address_type",
    "TEXT",
    "CVRUnit",
    "214fff73-0da6-48dc-ad8f-1e380fc3277c",
)
UNIT_PNUMBER = (
    "P-nummer",
    "org_unit_address_type",
    "PNUMBER",
    "PNumber",
    "8363e139-7ac1-4ab8-85cb-913ad443de33",
)
UNIT_EMAIL = (
    "Email",
    "org_unit_address_type",
    "EMAIL",
    "EmailUnit",
    "03300c9a-bd3f-4fad-b3fb-712c3b902d65",
)
UNIT_FAX = (
    "Fax",
    "org_unit_address_type",
    "PHONE",
    "FaxUnit",
    "d8563d4e-fabf-4704-becf-87292ce889b8",
)
UNIT_PHONE = (
    "Telefon",
    "org_unit_address_type",
    "PHONE",
    "PhoneUnit",
    "adb476ad-0860-4ec0-9df3-95f9fd4915b5",
)
UNIT_WEB = (
    "Webadresse",
    "org_unit_address_type",
    "WWW",
    "WebUnit",
    "74f658d4-094d-4b11-9d2a-d39d1031877b",
)
PERSON_EMAIL = (
    "Email",
...
PERSON_WORK_PHONE = (
    ...
    "PHONE",
    "WorkPhoneEmployee",
    "7169f171-f76c-4c9a-bee0-5e39e5c27eb2",
)
PERSON_MOBILE_PHONE = (
    "Mobiltelefon",
    "employee_address_type",
    "PHONE",
    "MobilePhoneEmployee",
    "225f5d0b-75f5-4f64-8f43-aede29f958a8",
)

UNKNOWN_JOB_FUNCTION = (
    "Ukendt stillingsbetegnelse",
    "engagement_job_function",
    None,
    None,
    "80f1d01e-60e1-4eca-9b36-932412de5a3b",
)
UNKNOWN_ORG_UNIT_TYPE = (
    "Ukendt enhedstype",
    "org_unit_type",
    None,
    None,
    "01aadec6-ec7e-4cc8-af76-1f582adda8f7",
)
...
class ÅrhusImport(object):
    ...
    def _import_engagement_types(self):
        ...
        for row in rows:
            ...
            self._import_klasse(engagementstype, "engagement_type", uuid=uuid)
    ...
    def _import_unit_types(self):
        ...
        for row in rows:
            ...
            self._import_klasse(enhedstype, "org_unit_type", uuid=uuid)

            ORG_UNIT_TYPE_CACHE[enhedstype.lower()] = uuid

        titel, facet, scope, bvn, uuid = UNKNOWN_ORG_UNIT_TYPE
        self._import_klasse(titel=titel, facet=facet, uuid=uuid)
    ...
    def _import_remaining_classes(self):
        ...
        for clazz in [
            ...
            self._import_klasse(
                titel=titel, facet=facet, scope=scope, bvn=bvn, uuid=uuid
            )
    ...
    def _parse_file(self, filename, uuid_fn):
        ...
        for org_uuid, history_list in object_history.items():
            object_history[org_uuid] = sorted(
                history_list, key=lambda x: x["from_date"]
            )
    ...
    async def _import_pdb_units(self, session):
        unit_info = self._parse_file_with_uuid(
            "Export_PDB_orgenhed.csv", "OrgenhedUUID"
        )
        ...
        for uuid, units in unit_info.items():
            ...
            payload = payloads.create_org_unit(
                ...
                org_unit_type_uuid=ORG_UNIT_TYPE_CACHE[
                    first_unit["Enhedstype"].lower()
                ],
    ...
    async def _import_los_units(self, session):
        unit_info = self._parse_file_with_uuid(
            "Export_LOS_orgenhed.csv", uuid_field="LOSUUID"
        )

        parent_map = {
            item[1][0]["ADM_ENHEDS_ID"]: item[0] for item in unit_info.items()
        }
        ...
        def consolidate_payloads(payload_list):
            ...
            for payload in payload_list[1:]:
                current_to_date = datetime.strptime(
                    current_payload["to_date"], "%Y-%m-%d"
                ) + timedelta(days=1)
                new_from_date = datetime.strptime(payload["from_date"], "%Y-%m-%d")

                if (
                    current_payload["name"] == payload["name"]
                    and current_payload["user_key"] == payload["user_key"]
                    and current_payload["parent_uuid"] == payload["parent_uuid"]
                    and current_payload["org_unit_type_uuid"]
                    == payload["org_unit_type_uuid"]
        ...
        for uuid, units in unit_info.items():
            ...
            for edit in edits:
                edit_payloads.setdefault(uuid, []).append(
                    payloads.edit_org_unit(**edit)
                )
                edit_payload_count += 1
    ...
    async def _import_los_addresses(self, session):
        ...
        def create_payload_dict(
            unit_row, org_unit_uuid, address_type_uuid, value_field
        ):
        ...
        def consolidate_payloads(payload_list):
            ...
            for payload in payload_list[1:]:
                ...
                if (
                    current_payload["value"] == payload["value"]
                    and current_to_date == new_from_date
                ):
                    current_payload["to_date"] = payload["to_date"]
                else:
        ...
        for address_type in adresses:
            ...
            *_, address_type_uuid = address_tuple
            for uuid, units in unit_info.items():
                payload_dicts = [
                    create_payload_dict(
                        unit_row, uuid, address_type_uuid, address_field
                    )
        ...
        for address_tuple in ADDRESS_TUPLES:
            ...
            for uuid, addresses in address_info.items():
                ...
                payload = payloads.create_address(
                    uuid=uuid,
                    address_type_uuid=address_type_uuid,
                    person_uuid=first_address[relname]
                    if relname == "PersonUUID"
                    else None,
                    org_unit_uuid=first_address[relname]
                    if relname == "OrgenhedUUID"
                    else None,
                ...
                for address in addresses[1:]:
                    payload = payloads.edit_address(
                        uuid=uuid,
                        address_type_uuid=address_type_uuid,
                        person_uuid=address[relname]
                        if relname == "PersonUUID"
                        else None,
                        org_unit_uuid=address[relname]
                        if relname == "OrgenhedUUID"
                        else None,
    ...
    async def initial_import(self):
        ...
        self.org_uuid = requests.get("http://localhost:5000/service/o/").json()[0][
            "uuid"
        ]
