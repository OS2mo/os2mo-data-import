...
from integrations.SD_Lon import sd, sd_mox  # noqa
...
def mo_request(service, method="get", **params):
    ...
    try:
        r = method(
            url, headers=headers, params=params, verify=sdmox_config["OS2MO_VERIFY"]
        )
    ...
    sdmox_config.update(
        {
            ...
            "AMQP_CHECK_WAITTIME": cfg.get(
                "integrations.SD_Lon.sd_mox.AMQP_CHECK_WAITTIME", 3
            ),
            "AMQP_CHECK_RETRIES": cfg.get(
...
def get_sdMox():
    ...
    classes = {
        i["user_key"]: i["uuid"]
        for i in mo_request("o/" + mora_org + "/f/org_unit_level/").json()["data"][
            "items"
        ]
    }
    ...
    classes = {
        i["user_key"]: i["uuid"]
        for i in mo_request("o/" + mora_org + "/f/time_planning/").json()["data"][
            "items"
        ]
    }
...
def ou_before_edit(data):
    ...
    if "name" in data["request"]["data"]:
        ...
        addresses = mo_request(
            "ou/" + data["uuid"] + "/details/address", at=from_date_str
        ).json()
        ...
        parent = mo_request(
            "ou/" + data["request"]["data"]["parent"]["uuid"], at=from_date_str
        ).json()
...
def register(app):
    ...
    Trigger.on(Trigger.ORG_UNIT, Trigger.RequestType.CREATE, Trigger.Event.ON_BEFORE)(
        ou_before_create
    )

    Trigger.on(Trigger.ORG_UNIT, Trigger.RequestType.EDIT, Trigger.Event.ON_BEFORE)(
        ou_before_edit
    )

    Trigger.on("address", Trigger.RequestType.CREATE, Trigger.Event.ON_BEFORE)(
        address_before_create
    )

    Trigger.on("address", Trigger.RequestType.EDIT, Trigger.Event.ON_BEFORE)(
        address_before_edit
    )
