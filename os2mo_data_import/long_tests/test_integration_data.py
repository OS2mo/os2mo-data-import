...
import requests
from fixture_generator.populate_mo import CreateDummyOrg, Size
...
class IntegrationDataTests(unittest.TestCase):
    ...
    def _run_import_and_test_org_sanity(
        self, extra_unit=0, extra_employee=0, extra_engagement=0
    ):
    ...
    def test_011_verify_existence_of_integration_data(self):
        ...
        integration_data = self.morah._mo_lookup(
            uuid, "ou/{}/integration-data", use_cache=False
        )
    ...
    def test_013_klasse_re_import(self):
        ...
        org = self.morah.read_organisation()
        classes = self.morah._mo_lookup(
            org, "o/{}/f/engagement_job_function/", use_cache=False
        )
    ...
    def test_030_add_forced_uuids(self):
        ...
        self.importer.add_organisation_unit(
            ...
            date_from=datetime.strftime(
                self.dummy_org.data.global_start_date, "%Y-%m-%d"
            ),
        )
        ...
        self._run_import_and_test_org_sanity(
            extra_unit=3, extra_employee=1, extra_engagement=2
        )

    @freeze_time("2018-12-03")
    def test_031_test_forced_uuid(self):
        unit = self.morah._mo_lookup(
