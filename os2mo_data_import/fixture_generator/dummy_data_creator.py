...
class CreateDummyOrg(object):
    ...
    def __init__(
        ...
        self.names = {
            "first": _load_names(path_to_names[0]),
    ...
    def create_user(self, manager=[], multiple_employments=False):
        ...
        if not multiple_employments:
            ...
            if random.random() > 0.75:
                ...
                time_to = None
            user.append(
                self._create_user(name, user_key, time_from, time_to, cpr, manager)
            )
        else:
            ...
            for i in range(1, random.randrange(1, 15)):
                ...
                user.append(self._create_user(name, user_key, time_from, time_to, cpr))
    ...
    def create_org_func_tree(self, org_size=Size.Normal):
        ...
        """
        orgs = [
            ("Borgmesterens Afdeling", "b6c11152-0645-4712-a207-ba2c53b391ab"),
        ...
        for node in list(keys):
            ...
            if not org_size == Size.Small:
                if org == "Teknik og Miljø":
                    orgs = [
                        ("Kloakering", "cf4daae1-4812-41f1-8c47-63a99e26aadf"),
                    ...
                    for uuid in uuids:
                        if random.random() > 0.5:
                            self._create_org_level(
                                ["Kantine"], self.nodes[uuid], level=2
                            )
                ...
                if org == "Borgmesterens Afdeling":
                    orgs = [
                        ...
                        ("HR og organisation", "96a4715c-f4df-422f-a4b0-9dcc686753f7"),
            ...
            if org == "Skole og Børn":
                ...
                if not org_size == Size.Small:
                    børnehaver = [dist + " børnehus" for dist in self._postdistrikter()]
                    uuids = self._create_org_level(
                        børnehaver, self.nodes[uuid], level=4
                    )
                    for uuid in uuids:
                        if random.random() > 0.5:
                            self._create_org_level(
                                ["Administration"], self.nodes[uuid], level=5
                            )
    ...
    def add_user_func(self, facet, node=None):
        ...
        if random.random() > 0.6:
            payload = {"unit": unit, "type": random.choice(self.classes[facet])}
        return payload
    ...
    def add_users_to_tree(self, ou_size_scale, multiple_employments=False):
        ...
        for key in list(keys):
            ...
            user_node = Node(
                name=user_info["user"][0]["givenname"],
...
if __name__ == "__main__":
    dummy_creator = CreateDummyOrg(825, "Læsø Kommune", _path_to_names())
    ...
    for node in PreOrderIter(dummy_creator.nodes["root"]):
        ...
        if node.type == "user":
            print()
            print("---")
            print("Given name: {}, Surname: {}".format(node.givenname, node.surname))
            print(node.parent.key)  # Key for parent unit
            ...
            for engagement in user:
                ...
                print("Fra: {}. Til: {}".format(engagement["fra"], engagement["til"]))
