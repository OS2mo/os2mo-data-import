...
class ImportUtility(object):
    ...
    def import_organisation(self, reference, organisation):
        ...
        integration_data = self._integration_data(
            resource=resource, reference=reference, payload={}
        )
    ...
    def import_klassifikation(self, reference, klassifikation):
        ...
        self.klassifikation_uuid = self.insert_mox_data(
            resource=resource, data=payload, uuid=klassifikation_uuid
        )
    ...
    def import_facet(self, reference, facet):
        ...
        self.inserted_facet_map[reference] = self.insert_mox_data(
            resource=resource, data=payload, uuid=facet_uuid
        )
    ...
    def import_itsystem(self, reference, itsystem):
        ...
        self.inserted_itsystem_map[reference] = self.insert_mox_data(
            resource=resource, data=payload, uuid=itsystem_uuid
        )
    ...
    def import_org_unit(self, reference, organisation_unit, details=[]):
        ...
        if re_import in ("YES", "NEW"):
            ...
            self.insert_mora_data(
                resource="service/details/create", data=details_payload
            )
    ...
    def import_employee(self, reference, employee, details=[]):
        ...
        uuid = self.insert_mora_data(resource=mora_resource, data=integration_data)
        ...
        if details:
            ...
            if re_import in ("YES", "NEW", "UPDATE"):
                self.insert_mora_data(
                    resource="service/details/create", data=additional_payload
                )
    ...
    def _integration_data(
        self, resource, reference, payload={}, encode_integration=True
    ):
        """
        ...
        if self.store_integration_data:
            ...
            payload["integration_data"] = self.ia.integration_data_payload(
                resource, reference, uuid, encode_integration
            )
        return payload
    ...
    def insert_mox_data(self, resource, data, uuid=None):
        ...
        if uuid:
            ...
            if response.status_code != 200:
                logger.error("Mox put. Response: {}, data".format(response.text, data))
            ...
            if response.status_code != 201:
                logger.error("Mox post. Response: {}, data".format(response.text, data))
    ...
    def insert_mora_data(self, resource, data, uuid=None):
        ...
        service_url = urljoin(base=self.mora_base, url=resource)
        ...
        response = self.session.post(
            url=service_url, headers=header, json=data, params=params
        )
        ...
        if response.status_code == 400:
            ...
            logger.error("MO post. Response: {}, data".format(response.text, data))
            raise HTTPError("Inserting mora data failed")
        else:
            uuid = response.json()
        return uuid
    ...
    def _terminate_details(self, uuid, detail_type):
        ...
        try:
            self.insert_mora_data(resource="service/details/terminate", data=payload)
        except HTTPError:
