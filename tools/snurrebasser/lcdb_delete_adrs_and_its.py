...
class SnurreBasse:
    ...
    def terminate(self, typ, uuid):
        response = self.helper._mo_post(
            "details/terminate",
            {"type": typ, "uuid": uuid, "validity": {"to": "2020-10-01"}},
        )
    ...
    def run_it(self):
        for i in (
            session.query(ItForbindelse.uuid)
            .filter(and_(ItForbindelse.bruger_uuid != None))
            .all()
        ):
            try:
                print(i)
    ...
    def run_adresse(self):
        for i in (
            session.query(Adresse.uuid)
            .filter(
                and_(Adresse.adressetype_scope == "E-mail", Adresse.bruger_uuid != None)
            )
            .all()
        ):
            try:
