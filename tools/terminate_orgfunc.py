...
def kill_it_connections(mh, lc):
    ...
    for uuid, itc in lc.it_connections.items():
        ...
        payload = {"type": "it", "uuid": uuid, "validity": {"to": terminate_date}}

        logger.debug("Terminate payload: {}".format(payload))
        response = mh._mo_post("details/terminate", payload)
        logger.debug(
            "Terminate response: {} for user {}".format(response.text, itc[0]["user"])
        )
...
def kill_addresses(mh, lc):
    ...
    for uuid, addr in lc.addresses.items():
        ...
        payload = {"type": "address", "uuid": uuid, "validity": {"to": terminate_date}}

        logger.debug("Terminate payload: {}".format(payload))
        response = mh._mo_post("details/terminate", payload)
        logger.debug(
            "Terminate response: {} for user {}".format(response.text, addr[0]["user"])
        )
