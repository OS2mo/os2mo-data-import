...
def export_ou_emus(mh, nodes, emus_file):
    ...
    rows = []
    for node in tqdm(
        cq.PreOrderIter(nodes["root"]), total=len(nodes), desc="export ou"
    ):
        ou = mh.read_ou(node.name)
        if not engagement_counter[ou["uuid"]]:
            logger.info(
                "skipping dept %s with no non-hourly-paid employees", ou["uuid"]
            )
            continue
...
def build_engagement_row(mh, ou, engagement):
    ...
    employee = mh._mo_lookup(engagement["person"]["uuid"], "e/{}")
    ...
    row = {
        ...
        "firstName": firstname,
        "lastName": lastname,
        "workPhone": _phone.get("name", "")
        if _phone.get("visibility", {}).get("scope", "") != "SECRET"
        else "hemmelig",
        "workContract": engagement.get("engagement_type", {}).get("uuid", ""),
        "workContractText": engagement.get("engagement_type", {}).get("name", ""),
        "positionId": engagement.get("job_function", {}).get("uuid", ""),
        "position": engagement.get("job_function", {}).get("name", ""),
        "orgUnit": ou["uuid"],
        "email": _email.get("name", "")
        if _email.get("visibility", {}).get("scope", "") != "SECRET"
        else "hemmelig",
        "username": username,
    }
    return row
...
def build_manager_rows(mh, ou, manager):
    ...
    employee = mh._mo_lookup(person["uuid"], "e/{}")
    ...
    if "surname" in person:
        ...
        firstname, lastname = person["name"].rsplit(" ", maxsplit=1)
...
def discarded(engagement):
    ...
    if etuuid not in EMUS_ALLOWED_ENGAGEMENT_TYPES:
        logger.debug(
            "%s discarded engagement_type %s", engagement["person"]["uuid"], etuuid
        )
        return True
    if jfkey in EMUS_DISCARDED_JOB_FUNCTIONS:
        logger.debug(
            "%s discarded job function %s", engagement["person"]["uuid"], jfkey
        )
        return True
    return False
