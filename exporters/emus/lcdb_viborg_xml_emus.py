...
def export_ou_emus(session, nodes, emus_file=sys.stdout):
    ...
    for node in tqdm(PreOrderIter(nodes["root"]), total=len(nodes), desc="export ou"):
        ...
        manager = session.query(Leder).filter(Leder.uuid == ou.leder_uuid).first()
        ...
        if manager_uuid:
            ...
            if entrydate is None:
                logger.info(
                    "skipping manager %s with no current employment (for ou)",
                    manager_uuid,
                )
                manager_uuid = ""
    ...
    for r in rows:
        ...
        if empls == 0:
            ...
            logger.debug(
                "department %s (%s) has %s engagements incl. subdepts.",
                r["longName"],
                r["uuid"],
                empls,
            )
...
def build_engagement_row(session, settings, ou, engagement):
    ...
    employee = session.query(Bruger).filter(Bruger.uuid == engagement.bruger_uuid).one()
...
def get_manager_dates(session, bruger_uuid):
    ...
    enddate = "0000-00-00"
    for engagement in (
        session.query(Engagement).filter(Engagement.bruger_uuid == bruger_uuid).all()
    ):
...
def build_manager_rows(session, settings, ou, manager):
    ...
    bruger = session.query(Bruger).filter(Bruger.uuid == manager.bruger_uuid).one()
    ...
    if entrydate is None:
        logger.info(
            "skipping manager %s with no current employment (for user)", manager.uuid
        )
        return []
    ...
    for responsibility in (
        session.query(LederAnsvar)
        .filter(
            LederAnsvar.leder_uuid == manager.uuid,
        )
        .all()
    ):
        if not responsibility.lederansvar_uuid == settings["EMUS_RESPONSIBILITY_CLASS"]:
...
def discarded(settings, engagement):
    ...
    if etuuid not in settings["EMUS_ALLOWED_ENGAGEMENT_TYPES"]:
        logger.debug("%s discarded engagement_type %s", engagement.bruger_uuid, etuuid)
        return True
...
def export_e_emus(session, settings, nodes, emus_file):
    ...
    settings["username-itsystem-uuid"] = (
        session.query(ItSystem.uuid)
        .filter(ItSystem.navn == "Active Directory")
        .scalar()
    )
    ...
    for node in tqdm(PreOrderIter(nodes["root"]), total=len(nodes), desc="export e"):
        ...
        engagements = (
            session.query(Engagement).filter(Engagement.enhed_uuid == ou.uuid).all()
        )
        ...
        for manager in session.query(Leder).filter(Leder.enhed_uuid == ou.uuid).all():
            if manager.bruger_uuid is None:
                ...
                manager_rows.extend(build_manager_rows(session, settings, ou, manager))

    if not len(manager_rows):
        logger.error(
            "no managers found - did You forget to"
    ...
    for r in rows:
        emus_file.write(
            '<employee id="%s" client="%s" lastChanged="%s">\n'
            % (
                r["employee_id"],
                r["client"],
                last_changed,
