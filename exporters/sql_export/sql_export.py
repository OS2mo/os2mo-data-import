...
from exporters.sql_export.sql_table_defs import (KLE, Adresse, Base, Bruger,
                                                 DARAdresse, Engagement, Enhed,
                                                 Enhedssammenkobling, Facet,
                                                 ItForbindelse, ItSystem,
                                                 Klasse, Kvittering, Leder,
                                                 LederAnsvar, Orlov, Rolle,
                                                 Tilknytning)
...
class SqlExport(object):
    def __init__(self, force_sqlite=False, historic=False, settings=None):
        ...
        if db_type == "SQLite":
            ...
            db_string = "mssql+pymssql://{}:{}@{}/{}".format(user, pw, db_host, db_name)
            ...
            db_string = "mysql+mysqldb://{}:{}@{}/{}".format(user, pw, db_host, db_name)
            ...
            for engagement_info in engagement_validity:
                ...
                sql_engagement = Engagement(
                    ...
                    engagementstype_titel=self.lc.classes[
                        engagement_info["engagement_type"]
                    ]["title"],
...
def cli():
    ...
    sql_export.perform_export(
        resolve_dar=args.get("resolve_dar"), use_pickle=args.get("use_pickle")
    )
